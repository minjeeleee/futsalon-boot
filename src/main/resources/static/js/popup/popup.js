	...
	fetch("/popup/team-info?tmCode="+tmCode)
		...
		.then(text => {
			if(text == 'disable'){
				...
				let tmLevel = (tmInfos[2]==='LC11'?'서울':tmInfos[2]==='LC31'?'경기':tmInfos[2]==='LC32'?'강원':tmInfos[2]==='LC33'?'충청':tmInfos[2]==='LC35'?'전라':tmInfos[2]==='LC39'?'제주':'경상');
				document.querySelector(".popup-tm-lc").innerHTML = '<span class="tit">활동지역</span>'+tmLevel;
